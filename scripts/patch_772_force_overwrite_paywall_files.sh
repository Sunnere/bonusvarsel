...
SERVICE_FILE="lib/services/paywall_trigger_service.dart"
SHEET_FILE="lib/widgets/premium_paywall_sheet.dart"

echo "==> patch_772_force_overwrite_paywall_files"

mkdir -p lib/services
mkdir -p lib/widgets

if [ -f "$SERVICE_FILE" ]; then
  cp "$SERVICE_FILE" "$SERVICE_FILE.bak_772_$(date +%Y%m%d_%H%M%S)"
fi
if [ -f "$SHEET_FILE" ]; then
  cp "$SHEET_FILE" "$SHEET_FILE.bak_772_$(date +%Y%m%d_%H%M%S)"
fi
...
echo "==> Verifiser at filene faktisk ble overskrevet"
echo "--- $SERVICE_FILE ---"
sed -n '1,220p' "$SERVICE_FILE"
echo
echo "--- $SHEET_FILE ---"
sed -n '1,260p' "$SHEET_FILE"
