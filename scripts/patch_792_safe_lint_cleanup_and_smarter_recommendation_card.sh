...
echo "==> patch_792_safe_lint_cleanup_and_smarter_recommendation_card"

PAYWALL_FILE="lib/services/paywall_trigger_service.dart"
WIDGET_FILE="lib/widgets/best_recommendation_card.dart"

if [ ! -f "$PAYWALL_FILE" ]; then
  echo "❌ Fant ikke $PAYWALL_FILE"
  exit 1
fi

if [ ! -f "$WIDGET_FILE" ]; then
  echo "❌ Fant ikke $WIDGET_FILE"
  exit 1
fi

cp "$PAYWALL_FILE" "$PAYWALL_FILE.bak_792_$(date +%Y%m%d_%H%M%S)"
cp "$WIDGET_FILE" "$WIDGET_FILE.bak_792_$(date +%Y%m%d_%H%M%S)"
echo "✅ Backup laget"

python3 <<'PY'
from pathlib import Path

paywall_path = Path("lib/services/paywall_trigger_service.dart")
...
# --- 1) Safe lint cleanup in paywall trigger service ---
paywall_old = "      if (allowed && context != null && context.mounted) {"
paywall_new = "      if (allowed && context.mounted) {"
if paywall_old in paywall:
    paywall = paywall.replace(paywall_old, paywall_new, 1)
    report.append("ryddet unødvendig null-sjekk i paywall_trigger_service.dart")
else:
    report.append("ingen null-sjekk endret i paywall_trigger_service.dart")

paywall_path.write_text(paywall)

# --- 2) Rewrite recommendation widget with smarter premium/elite behavior ---
widget_rewrite = """import 'package:flutter/material.dart';
...
"""
widget_path.write_text(widget_rewrite)
report.append("overskrev best_recommendation_card.dart med premium/elite-smartere versjon")
report.append("ryddet unødige string interpolation-braces ved full rewrite")

Path("lib/services/_patch_792_report.txt").write_text("\\n".join(report) + "\\n")
print("\\n".join(report))
PY

echo
echo "==> Rapport"
cat lib/services/_patch_792_report.txt || true
