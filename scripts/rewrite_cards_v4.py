...
lines += [
    ...
    "    return Scaffold(",
    "      backgroundColor: const Color(0xFFCFE2F3),",  # blåere bakgrunn
    "      appBar: AppBar(",
...
lines += [
    ...
    "        border: Border.all(",
    "          color: isSelected ? card.activeColor : const Color(0xFFBFDBFE),",
...
lines.append("                  style: TextStyle(color: card.activeColor,")
lines.append("                    fontSize: 12, fontWeight: FontWeight.w700)),")
...
import subprocess
# Verifiser nøkkelpunkter
checks = [
    (['grep', '-c', 'CustomPainter', path], 'CustomPainter-klasser'),
    (['grep', 'card.network', path],         'Interpolation'),
    (['grep', 'Annonse', path],               'Annonseplass'),
    (['grep', 'AnimatedContainer', path],     'AnimatedContainer (fargeskift)'),
    (['grep', 'Color(0xFFCFE2F3)', path],     'Blåere bakgrunn'),
]
for cmd, label in checks:
    r = subprocess.run(cmd, capture_output=True, text=True)
    status = '✅' if r.stdout.strip() else '❌'
    print(f'{status} {label}: {r.stdout.strip()[:60]}')

print()
print('✅ cards_page.dart v4 ferdig')
