...
echo "==> 722_force_replace_travel_page_with_visible_offers_block"
...
if repo_candidates:
    ...
    for name in preferred_methods:
        if re.search(rf"\b{name}\s*\(", repo_text):
            repo_method_name = name
            break

stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
bak = travel_path.with_name(travel_path.name + f".bak_{stamp}_722")
shutil.copy2(travel_path, bak)
print(f"Backup: {bak}")
print(f"Offers repo import: {repo_import_line or 'NOT FOUND'}")
print(f"Offers repo method: {repo_method_name or 'NOT FOUND'}")

method_call = f"OffersFeedRepository().{repo_method_name}()" if repo_import_line and repo_method_name else "null"
...
class _TravelPageState extends State<TravelPage> {{
  ...
  Future<List<_TravelOfferSuggestion>> _loadTripFeedSuggestions() async {{
    try {{
      final dynamic response = {method_call};
      if (response == null) {{
  ...
  List<_StoreSuggestion> _buildStoreSuggestions(List<_PackingNeed> needs) {{
    ...
    if (categories.contains('Bagasje')) {{
      results.add(
        const _StoreSuggestion(
          ...
          why: 'Høy relevans for familie, flere kolli og organisering.',
        ),
      );
    }}
    ...
    if (categories.contains('Helse / apotek')) {{
      results.add(
        const _StoreSuggestion(
          ...
          why: 'Bra for solkrem, reiseapotek og hygiene før tur.',
        ),
      );
    }}
    ...
    if (categories.contains('Sport / fritid') ||
      ...
      results.add(
        const _StoreSuggestion(
          ...
          why: 'God match for strandutstyr, vinterutstyr og aktiv ferie.',
        ),
      );
    }}
    ...
    if (categories.contains('Klær') || categories.contains('Sko / klær')) {{
      results.add(
        const _StoreSuggestion(
          ...
          why: 'Relevant for badetøy, gåsko, ull og familieklær.',
        ),
      );
    }}
    ...
    if (categories.contains('Elektronikk')) {{
      results.add(
        const _StoreSuggestion(
          ...
          why: 'Bra for powerbank, ladere og reiseteknologi.',
        ),
      );
    }}
    ...
    if (_children > 0) {{
      results.add(
        const _StoreSuggestion(
          title: 'Barn og familiebutikk',
          why: 'Smart for småbarnsutstyr, aktiviteter og familietilbehør.',
        ),
      );
    }}
    ...
    if (results.isEmpty) {{
      results.add(
        _StoreSuggestion(
          ...
          why: 'Se etter kategorier som matcher: ${{hints.join(', ')}}.',
        ),
      );
    }}
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 4),
          Text('Antall: ${{need.quantity}}'),
          Text('Prioritet: ${{need.priority}}'),
          Text('Kategori: ${{need.category}}'),
          Text('Se etter butikker i appen for: ${{need.storeHint}}'),
        ],
      ),
    );
  }}
...
"""
travel_path.write_text(content)
...
echo "  grep -n \"Butikkforslag fra offers feed\" lib/pages/travel_page.dart"
