...
BACKUP="${FILE}.bak_${STAMP}_823b"

echo "==> 823b_fix_premium_page_precise"
cp "$FILE" "$BACKUP"
echo "Backup: $BACKUP"
...
def must_find(needle: str, label: str):
    if needle not in src:
        print(f"❌ Fant ikke {label}")
        sys.exit(1)

# 1) Import checkout_page.dart tilbake
if "import 'checkout_page.dart';" not in src:
    needle = "import '../services/checkout_service.dart';\n"
    must_find(needle, "import for checkout_service.dart")
    src = src.replace(
        needle,
...
# 2) Reopen checkout for Premium/Elite
...
"""
must_find(old_checkout, "_checkout-blokken")
src = src.replace(old_checkout, new_checkout, 1)

# 3) Clean partner fallback
old_partner = """  Future<void> _openPartnerUrl(String url) async {
...
new_partner = """  Future<void> _openPartnerUrl(String url) async {
    final uri = Uri.parse(url);
    if (!await launchUrl(uri, mode: LaunchMode.externalApplication)) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
...
"""
must_find(old_partner, "_openPartnerUrl-blokken")
src = src.replace(old_partner, new_partner, 1)

# 4) Build header with dynamic look vars
...
new_build_header = """  @override
  Widget build(BuildContext context) {
    ...
    return Scaffold(
"""
must_find(old_build_header, "build-header")
...
new_hero = """                    _HeroConversion(
                      ...
                      chips: const [
"""
must_find(old_hero, "_HeroConversion-blokken")
...
"""
must_find(old_valuebar, "_ValueBar-blokken")
src = src.replace(old_valuebar, new_valuebar, 1)

# 7) Bottom CTA text and colors
...
"""
must_find(old_cta, "nederste CTA-blokken")
