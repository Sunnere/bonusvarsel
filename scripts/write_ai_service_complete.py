...
path = os.path.expanduser('~/bonusvarsel/lib/services/ai_service.dart')
with open(path, 'r') as f:
    orig = f.read()
with open(path + '.bak_full', 'w') as f:
    f.write(orig)

dart = r"""import 'dart:convert';
...
class AiService {
  ...
  static Future<String> getTravelPlan({
    ...
    if (apiKey.isEmpty) {
      return '❌ API-nøkkel mangler. Kjør med --dart-define=ANTHROPIC_API_KEY=din-nøkkel';
    }
    ...
    final prompt = '''
Du er ekspert på norske bonusprogram. Svar på norsk. Vær konkret med tall. Ingen markdown-tabeller.
...
Vær spesifikk med tall og kronbeløp.''';
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return '❌ API-feil ${response.statusCode}. Kontakt $_supportEmail';
      }
    } catch (e) {
  ...
  static Future<String> sendMessage(
    ...
    final apiKey = await _getApiKey();
    if (apiKey.isEmpty) {
      return '❌ API-nøkkel mangler. Kontakt $_supportEmail';
    }
...
import subprocess
checks = ['getTravelPlan', 'sendMessage', '_buildCardDetails',
          '_destPointsCost', '_getApiKey', 'companion ticket']
all_ok = True
for c in checks:
    r = subprocess.run(['grep', '-c', c, path], capture_output=True, text=True)
    ok = int(r.stdout.strip()) > 0
    if not ok: all_ok = False
    print(f"  {'✅' if ok else '❌'} {c}")
print()
print("✅ ai_service.dart ferdig" if all_ok else "❌ Sjekk feil over")
