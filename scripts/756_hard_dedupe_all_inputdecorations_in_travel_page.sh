...
echo "==> 756_hard_dedupe_all_inputdecorations_in_travel_page"
...
bak = path.with_name(path.name + f".bak_{stamp}_756")
...
def dedupe_named_args(inner: str) -> str:
...
for needle in ["InputDecoration(", "const InputDecoration("]:
    pass

i = 0
while i < len(text):
    next_plain = text.find("InputDecoration(", i)
    next_const = text.find("const InputDecoration(", i)

    candidates = [x for x in [next_plain, next_const] if x != -1]
    if not candidates:
        parts.append(text[i:])
        break
    ...
    if text.startswith("const InputDecoration(", start):
        ...
        open_idx = start + len("const InputDecoration") 
    else:
    ...
    fixed_inner = dedupe_named_args(inner)
...
# ekstra cleanup for gamle restlinjer
cleanup_patterns = [
    (r"\n\s*border:\s*OutlineInputBorder\(\),\s*\n\s*border:\s*OutlineInputBorder\(\),", "\n                  border: OutlineInputBorder(),"),
    (r"\n\s*filled:\s*true,\s*\n\s*filled:\s*true,", "\n                  filled: true,"),
    (r"\n\s*fillColor:\s*const Color\(0xFFF4F7FB\),\s*\n\s*fillColor:\s*const Color\(0xFFF4F7FB\),", "\n                  fillColor: const Color(0xFFF4F7FB),"),
]
for pat, repl in cleanup_patterns:
    new_text = re.sub(pat, repl, new_text, flags=re.DOTALL)
